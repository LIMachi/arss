...
public class InstrumentSwapperMenu extends AbstractContainerMenu {
    ...
    @Override
    public boolean stillValid(Player player) { return stillValid(accessor, player, com.limachi.arss.common.blocks.InstrumentSwapper.R_BLOCK.get()); }

    public static void open(Player player, InstrumentSwapper be) {
        if (player instanceof ServerPlayer serverPlayer)
            MenuRegistry.openExtendedMenu(serverPlayer, new MenuProvider() {
                ...
                public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
                    return new InstrumentSwapperMenu(id, inventory, be, be.getBlockPos());
                }
            }, b->{});
    }
}
